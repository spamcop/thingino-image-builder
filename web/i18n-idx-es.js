// Spanish strings for the public builder page: same keys as the English baseline.
I18N.add("es", {
  // index.html: estático
  page_title: "thingino · Constructor de Imágenes",
  header_full: " Constructor de Imágenes Thingino",
  header_short: " Constructor de Imágenes Thingino",
  commit_badge_title: "commit de thingino en compilación",
  connecting: "Conectando…",
  camera_label: "defconfig de cámara",
  board_placeholder: "empieza a escribir, p. ej. atom_cam2…",
  build_btn: "Compilar",
  loading_cameras: "Cargando lista de cámaras…",
  footer_limits: "Las compilaciones se ejecutan en GitHub Actions (~20–40 min), así que ten paciencia. Límites de usuario: <strong>{user} compilaciones/hora</strong> por usuario, <strong>{conc}</strong> en total a la vez. Las imágenes terminadas se pueden descargar durante <strong>{mins} minutos</strong>, después se eliminan.",
  footer_link: "constructor de imágenes thingino",
  admin_link: "admin",
  // app.js: dinámico
  min_approx: "~{n} min",
  not_known_defconfig: "No es un defconfig conocido.",
  profiles_available: "{n} perfiles de cámara disponibles.",
  cameras_load_failed: "No se pudo cargar la lista de cámaras, ¿está activo el broker?",
  stats_building: "compilando",
  stats_queued: "en cola",
  stats_typical: "compilación típica",
  commit_badge_text: "{branch} @ {commit}",
  builds_disabled: "Las compilaciones están desactivadas temporalmente por mantenimiento en curso, vuelve más tarde.",
  // Notice banner: the level label the admin picked (the notice text itself is
  // theirs, shown as written). Names and colours follow GitHub's markdown alerts.
  notice_note: "Nota",
  notice_tip: "Consejo",
  notice_important: "Importante",
  notice_warning: "Advertencia",
  notice_caution: "Precaución",
  meta_defconfig: "defconfig",
  meta_build_id: "id de compilación",
  deduped_note: "se reutilizó una imagen idéntica existente",
  state_queued: "En cola",
  queued_position: "(posición {n})",
  cancel_btn: "Cancelar",
  state_building: "Compilando…",
  state_cancelling: "Cancelando…",
  cancelling_note: "deteniendo la compilación, esto puede tardar un momento",
  state_done: "Compilación lista",
  download_btn: "Descargar .bin",
  build_another_btn: "Compilar otra",
  download_window_note: "Disponible durante ~{mins} minutos, después se elimina.",
  state_failed: "Compilación fallida",
  try_again_btn: "Reintentar",
  state_expired: "Ventana de descarga expirada",
  state_cancelled: "Cancelada",
  build_again_btn: "Compilar de nuevo",
  request_failed: "La solicitud falló ({status}).",
  gear_title: "Ajustes",
  settings_title: "Ajustes",
  settings_lang: "Idioma",
  btn_cancel: "Cancelar",
  btn_save: "Guardar",
  branch_label: "rama de thingino desde la que compilar:",
  branch_default: "(predeterminada)",
  branch_note: "Cambia la rama de thingino desde la que se compila tu firmware. La lista de cámaras se actualiza en consecuencia.",
  // diálogo Acerca de (el botón de información de la cabecera)
  about_title: "Acerca de",
  about_what: "thingino Image Builder compila el firmware de tu cámara en la nube. Elige el modelo de tu cámara y pulsa Compilar: la imagen se compila desde el código fuente en GitHub Actions, así que no hay ninguna cuenta que crear, ningún toolchain que instalar ni nada que configurar.",
  about_project: "thingino es firmware libre y de código abierto para cámaras IP basadas en Ingenic, hecho por una comunidad de voluntarios. La capacidad de compilación se comparte entre todas las personas que usan esta página, así que se aplican límites por hora y las imágenes terminadas solo se conservan un rato; ambos datos aparecen al pie del compilador.",
  about_caution: "Instalar un firmware nuevo puede dejar la cámara inservible. Lee la página wiki de tu cámara y haz una copia de seguridad de su flash original antes de empezar.",
  about_hints: "Las sugerencias al pasar el cursor por cada control se activan en Ajustes.",
  about_links: "Enlaces del proyecto",
  about_site: "Sitio del proyecto thingino",
  about_wiki: "Documentación y wiki de cámaras",
  about_firmware: "Código fuente del firmware thingino",
  about_telegram: "Grupo de Telegram",
  about_discord: "Servidor de Discord",
  about_source: "Código fuente de este compilador",
  about_version: "Versión del compilador",
  btn_close: "Cerrar",
  // help balloons
  help_about: "Acerca de este compilador: qué hace y dónde vive el proyecto.",
  help_board: "Empieza a escribir el modelo de tu cámara para encontrar su perfil de compilación.",
  help_build: "Compila el firmware de la cámara seleccionada, se ejecuta en GitHub Actions, ~20–40 min.",
  help_stats: "Estado en vivo: compilaciones en curso, cuántas hay en cola y el tiempo de compilación típico.",
  help_commit: "El commit exacto de thingino en el que está esta rama, haz clic para abrirlo en GitHub.",
  help_lang: "Cambia el idioma de la interfaz (detectado automáticamente de tu navegador).",
  help_settings: "Ajustes: elige desde qué rama de thingino compilar.",
  help_branch: "Elige la rama de thingino desde la que compilar. La lista de cámaras se actualiza en consecuencia.",
  setting_help_label: "Muestra sugerencias de ayuda: pasa el cursor sobre cualquier control para ver un consejo.",
  privacy_link: "privacidad",
  // share links (?board=&branch=) + the copy-link button
  share_btn: "Copiar enlace",
  share_copied: "Copiado",
  help_share: "Copia un enlace que abre esta página con esta cámara y esta rama ya seleccionadas.",
  link_bad_board: "El nombre de cámara de ese enlace no es válido, elige una cámara abajo.",
  link_bad_branch: "Ese enlace indica una rama desconocida, así que se ha seleccionado la tuya habitual. Ramas conocidas: {refs}.",
  link_not_on_branch: "{board} no está disponible en {branch}.",
  link_switch: "Cambiar a {branch}",
  link_not_anywhere: "{board} no está disponible en ninguna rama.",
  branch_gone: "{was} ya no está disponible, así que se ha seleccionado {branch} en su lugar.",
  dismiss_btn: "Descartar",
});
